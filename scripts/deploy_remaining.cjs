...
async function main() {
    ...
    // 3. Link GyeManager in ScoreManager if possible
    // (Assuming ScoreManager is already deployed and we have permission, or we just skip this for now relative to the user request)

    // 4. Wait for block confirmations
    console.log("\n⏳ Waiting for 5 block confirmations for verification...");
    await gyeManager.deploymentTransaction().wait(5);
